...
const TRACKS: Track[] = [
  ...
  // extra (si quieres usar los otros 2 que bajaste)
  { id: 'naturaleza', name: '🏞️ Naturaleza', src: '/audio/naturewater.mp3' },
  { id: 'avion', name: '✈️ Avión', src: '/audio/airplane.mp3' },
];
...
export default function AudioMeditation() {
  ...
  // Cuando cambias de track, reinicia todo
  useEffect(() => {
  ...
  const start = async () => {
    ...
    if (!a) return;
    try {
      // iPhone/Android requieren interacción del usuario: esto se llama desde un botón, OK
      await a.play();
      setPlaying(true);

      // si hay timer activo y estaba en 0, lo arrancamos
      if (timerTotal && !timerLeft) setTimerLeft(timerTotal);
    } catch (e) {
      console.log(e);
    }
  };
    ...
    <div className="card" style={{ marginTop: 12 }}>
      ...
      <div className="card" style={{ marginTop: 12 }}>
        ...
        <div className="row" style={{ gap: 10, flexWrap: 'wrap' }}>
          ...
          <button className="pill" onClick={() => setTimer(30)}>
            30 min
          </button>
        ...
        <div className="small" style={{ marginTop: 10 }}>
          {timerTotal ? (
            <>
              Se apaga en: <b>{fmt(timerLeft || timerTotal)}</b>
            </>
          ) : (
            <>Sin timer activado (se queda sonando en loop).</>
          )}
        </div>
      </div>

      <div className="small" style={{ marginTop: 10, opacity: 0.9 }}>
        Tip: si no se escucha, sube el volumen del teléfono y prueba “Reproducir” otra vez.
      </div>
    </div>
  );
}
